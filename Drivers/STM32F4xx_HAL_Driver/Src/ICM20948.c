...
uint16_t ICM_Initialize(void)
{
	...
	HAL_Delay(20);
	ICM_SetGyroRateLPF(GYRO_FS_250, GYRO_LPF_17HZ);
	HAL_Delay(10);
...
void ICM_ReadAccel(struct ICM_20948 *IMU)
{
	uint8_t raw_data[6];
	ICM_readBytes(0x2D, raw_data, 6);

	IMU->accel.x = (raw_data[0] << 8) | raw_data[1] / IMU->accel.sensitivity;
	IMU->accel.y = (raw_data[2] << 8) | raw_data[3] / IMU->accel.sensitivity;
	IMU->accel.z = (raw_data[4] << 8) | raw_data[5] / IMU->accel.sensitivity;

}

void ICM_ReadGyro(struct ICM_20948 *IMU)
{
	uint8_t raw_data[6];
	ICM_readBytes(0x33, raw_data, 6);

	IMU->gyro.x = (raw_data[0] << 8) | raw_data[1] / IMU->gyro.sensitivity;
	IMU->gyro.y = (raw_data[2] << 8) | raw_data[3] / IMU->gyro.sensitivity;
	IMU->gyro.z = (raw_data[4] << 8) | raw_data[5] / IMU->gyro.sensitivity;
